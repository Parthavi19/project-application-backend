...
def extract_json(response: str) -> dict:
    ...
    # Remove markdown code fences, language hints, and extra content
    ...
    logging.info("Cleaned AI response: %s", cleaned_response)
    ...
    try:
        ...
        if not isinstance(parsed, dict) or not all(key in parsed for key in expected_keys):
            logging.error(f"Parsed JSON missing required keys: {expected_keys}")
    ...
    # Try fuzzy-json parsing as fallback
    try:
        parsed = fuzzy_loads(cleaned_response)
        # Validate expected keys
        expected_keys = {"html", "css", "js", "name"}
        if not isinstance(parsed, dict) or not all(key in parsed for key in expected_keys):
            logging.error(f"Fuzzy-parsed JSON missing required keys: {expected_keys}")
            raise ValueError(f"Parsed JSON does not contain expected keys: {expected_keys}")
...
def generate_code_from_prompt(prompt: str, api_key: str = None, retries=2) -> str:
    formatted = (
        ...
        "  \"name\": \"App Name\"\n"
        "}\n"
        "IMPORTANT: Do not add any markdown formatting (like ```json) or explanations. Only return pure JSON with keys: html, css, js, name."
    )
